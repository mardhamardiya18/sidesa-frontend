<template>
  <div id="Header" class="flex items-center justify-between">
    <div class="flex flex-col gap-2">
      ...
      <h1 class="font-semibold text-2xl">Update Event Desa</h1>
    </div>
  </div>
  <form @submit.prevent="handleSubmit" id="myForm" class="capitalize">
    <div class="shrink-0 rounded-3xl p-6 bg-white flex flex-col gap-6 h-fit">
      ...
      <section id="Buttons" class="flex items-center justify-end gap-4">
        <router-link :to="{ name: 'manage-event' }">
          <div
...
const { fetchEvent, fetchEventUpdate } = eventStore;
const { loading, success } = storeToRefs(eventStore);
...
const loadData = async () => {
  ...
  payload.value = res;
  payload.value.thumbnail_url = res.thumbnail;
