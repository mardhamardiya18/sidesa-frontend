<template>
  ...
  <section id="List-Kepala-Rumah" class="flex flex-col gap-[14px]">
    ...
    <CardList
      v-for="(headOfFamily, index) in headOfFamilies"
      :key="index"
      :item="headOfFamily"
      v-if="!loading"
    />
...
const headOfFamiliesStore = useHeadOfFamilyStore();
const { headOfFamilies, meta, loading, error, success } =
  storeToRefs(headOfFamiliesStore);
const { fetchHeadOfFamiliesPaginated } = headOfFamiliesStore;
